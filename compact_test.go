...
func TestMergeOverlappingGroupChunks(t *testing.T) {
	...
	dirs := []string{
		...
		createBlock(t, tmpdir, genSeriesOrderedPlusOne(3, 2, 50, 200), 3),
		createBlock(t, tmpdir, genSeriesOrderedPlusTwo(3, 2, 150, 300), 3),
	}
	blks := []*Block{}
	b, err := OpenBlock(nil, dirs[0], nil)
	testutil.Ok(t, err)
	blks = append(blks, b)
	b, err = OpenBlock(nil, dirs[1], nil)
	testutil.Ok(t, err)
	blks = append(blks, b)
	b, err = OpenBlock(nil, dirs[2], nil)
	testutil.Ok(t, err)
	blks = append(blks, b)
	defer func() {
		testutil.Ok(t, blks[0].Close())
		testutil.Ok(t, blks[1].Close())
		testutil.Ok(t, blks[2].Close())
	}()
	...
	testutil.Ok(t, err)
	testutil.Equals(t, 3, len(mergedChks))
	testutil.Equals(t, 1, len(mergedChks[0]))

	for _, chk := range mergedChks {
		it := chk[0].Chunk.IteratorGroup(0, int(chk[0].SeriesRef))
		time := int64(0)
		for it.Next() {
			t_, v := it.At()
			testutil.Equals(t, time, t_)
			if time >= 150 {
				testutil.Equals(t, float64(time + 2), v)
			} else if time >= 50 {
				testutil.Equals(t, float64(time + 1), v)
			} else {
				testutil.Equals(t, float64(time), v)
			}
			time += 1
		}
		testutil.Equals(t, int64(300), time)
	}

	testutil.Equals(t, false, cm.Next())
}
...
func TestCompactAll(t *testing.T) {
	...
	dirs := []string{
		createBlock(t, tmpdir, genSeriesOrdered(3, 2, 0, 100), 3),
		createBlock(t, tmpdir, genSeriesOrdered(6, 2, 150, 200), 3),
		createBlock(t, tmpdir, genSeriesOrdered(9, 2, 200, 300), 3),
	}
	blks := []*Block{}
	b, err := OpenBlock(nil, dirs[0], nil)
	testutil.Ok(t, err)
	...
	{
		// Test blockQuerier for block 1.
		bq, err := NewBlockQuerier(blks[0], 25, 77)
		testutil.Ok(t, err)
		...
		// LabelValues.
		lvals, err := bq.LabelValues("labelName")
		testutil.Ok(t, err)
		testutil.Equals(t, 3, len(lvals))
		for i := 0; i < len(lvals); i++ {
			testutil.Equals(t, strconv.Itoa(i), lvals[i])
		}
		lvals, err = bq.LabelValues("labelName1")
		testutil.Ok(t, err)
		testutil.Equals(t, 1, len(lvals))
		testutil.Equals(t, "labelValue1", lvals[0])

		// Select.
		ss, err := bq.Select(labels.NewEqualMatcher("labelName1", "labelValue1"))
		testutil.Ok(t, err)
		numSeries := 0
		for ss.Next() {
			s := ss.At()
			...
			testutil.Equals(t, int64(78), time)

			numSeries += 1
		}
		testutil.Equals(t, 3, numSeries)
	...
	}
	{
		// Test querier.
		bqs := []Querier{}
		bq, err := NewBlockQuerier(blks[0], blks[0].MinTime(), blks[0].MaxTime())
		testutil.Ok(t, err)
		bqs = append(bqs, bq)
		bq, err = NewBlockQuerier(blks[1], blks[1].MinTime(), blks[1].MaxTime())
		testutil.Ok(t, err)
		bqs = append(bqs, bq)
		bq, err = NewBlockQuerier(blks[2], blks[2].MinTime(), blks[2].MaxTime())
		testutil.Ok(t, err)
		bqs = append(bqs, bq)

		q := querier{blocks: bqs}
		{
			// Test the first 3 series.
			matcher, err := labels.NewRegexpMatcher("labelName", "[0-2]")
			testutil.Ok(t, err)
		...
		{
			// Test the series 3-5.
			matcher, err := labels.NewRegexpMatcher("labelName", "[3-5]")
			testutil.Ok(t, err)
			ss, err := q.Select(matcher)
			testutil.Ok(t, err)
		...
		{
			// Test the series 6-8.
			matcher, err := labels.NewRegexpMatcher("labelName", "[6-8]")
			testutil.Ok(t, err)
			ss, err := q.Select(matcher)
			testutil.Ok(t, err)
	...
	}
	{
		// Test Plan().
		{
			c, err := NewLeveledCompactor(context.Background(), nil, nil, []int64{100, 199}, nil)
			testutil.Ok(t, err)

			planDirs, err := c.Plan(tmpdir)
			testutil.Ok(t, err)
			testutil.Equals(t, 2, len(planDirs))
			testutil.Equals(t, blks[0].Dir(), planDirs[0])
			testutil.Equals(t, blks[1].Dir(), planDirs[1])
		}
		{
			c, err := NewLeveledCompactor(context.Background(), nil, nil, []int64{100, 198}, nil)
			testutil.Ok(t, err)

			planDirs, err := c.Plan(tmpdir)
			testutil.Ok(t, err)
			testutil.Equals(t, 0, len(planDirs))
		}
		{
			c, err := NewLeveledCompactor(context.Background(), nil, nil, []int64{100, 200}, nil)
			testutil.Ok(t, err)
	...
	}
	{
		...
		ulid, err := c.Compact(tmpdir, planDirs, nil)
		testutil.Ok(t, err)

		b, err := OpenBlock(nil, filepath.Join(tmpdir, ulid.String()), nil)
		testutil.Ok(t, err)

		// Test blockQuerier for compacted block.
		bq, err := NewBlockQuerier(b, b.MinTime(), b.MaxTime())
		testutil.Ok(t, err)
		...
		for ss.Next() {
			...
			it := s.Iterator()
			time := int64(0)
			if numSeries == 3 || numSeries == 4 || numSeries == 5 {
				time = 150
			}
			for it.Next() {
				t_, v := it.At()
				...
				testutil.Equals(t, float64(time), v)
				time += 1
				if time == 100 && (numSeries == 0 || numSeries == 1 || numSeries == 2) {
					time = 150
				}
			}
			testutil.Equals(t, int64(200), time)
...
// func (nopChunkWriter) WriteChunks(chunks ...chunkenc.Meta) error { return nil }
