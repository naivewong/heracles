...
type ChunkSeriesSet interface {
	Next() bool
	At() (labels.Labels, []chunkenc.Meta, Intervals)
	Err() error
}

type GroupChunkSeriesSet interface {
	Next() bool
	At() ([]labels.Labels, [][]chunkenc.Meta, Intervals) // Each row represents one series.
	Err() error
}
...
// NOTE(Alec), baseChunkSeries fine-grained filters the chunks using tombstone.
type baseChunkSeries struct {
	...
	intervals Intervals
	err       error
	// group     bool // To determine if this 
}
...
func (s *baseChunkSeries) At() (labels.Labels, []chunkenc.Meta, Intervals) {
...
func (s *baseChunkSeries) Next() bool {
	var (
		...
		chkMetas = make([]chunkenc.Meta, len(s.chks))
		err      error
	)
	...
	for s.p.Next() {
		...
		s.chks = chkMetas
		if len(s.chks) > 0 {
			s.intervals, err = s.tombstones.Get(s.chks[0].LogicalGroupRef) // Should use the group ref.
			if err != nil {
		...
		if len(s.intervals) > 0 {
			...
			chks := make([]chunkenc.Meta, 0, len(s.chks))
...
// given time range.
//
// NOTE(Alec), PopulatedChunkSeriesSet coarse-grained filters the chunks using mint and maxt.
...
func (s *populatedChunkSeries) At() (labels.Labels, []chunkenc.Meta, Intervals) {
	return s.lset, s.chks, s.intervals
}
...
type chunkSeries struct {
	labels labels.Labels
	chunks []chunkenc.Meta // in-order chunk refs
...
// SeriesIterator iterates over the data of a time series of a group.
...
type GroupSeriesIterator interface {
	Seek(t int64) bool
	At() (t int64, v []float64)
	Next() bool
	Err() error
...
// Contains only one series in the group.
type chunkSeriesIterator struct {
	chunks []chunkenc.Meta
...
func newChunkSeriesIterator(cs []chunkenc.Meta, dranges Intervals, mint, maxt int64) *chunkSeriesIterator {
	it := cs[0].Chunk.IteratorGroup(mint, int(cs[0].SeriesRef))
...
func (it *chunkSeriesIterator) Seek(t int64) (ok bool) {
	...
	// Need to get new iterator if it.i forwards.
	if it.i != last {
		it.cur = it.chunks[it.i].Chunk.IteratorGroup(it.mint, int(it.chunks[it.i].SeriesRef))
		if len(it.intervals) > 0 {
			it.cur = &deletedIterator{it: it.cur, intervals: it.intervals}
		}
	}

	return it.cur.Seek(t) // I have added Seek() in chunkenc.Iterator.
}
...
// deletedIterator wraps an Iterator and makes sure any deleted metrics are not returned.
//
// NOTE(Alec), this implements chunkenc.Iterator (not SeriesIterator).
//
// NOTE(Alec), in my implementation, the intervals should be sorted.
...
Outer:
	for it.it.Next() {
		...
		for it.i != len(it.intervals) {
			if it.intervals[it.i].inBounds(ts) {
				continue Outer
			}

			if ts > it.intervals[it.i].Maxt {
				it.i += 1
				continue
			}
...
func (it *deletedIterator) Seek(x int64) bool {
	if !it.it.Seek(x) {
		return false
	}
	for {
...
// Implement GroupSeriesIterator.
type groupChunkSeriesIterator struct {
	chunks [][]chunkenc.Meta

	i   int
	cur GroupSeriesIterator
...
func newGroupChunkSeriesIterator(cs [][]chunkenc.Meta, dranges Intervals, mint, maxt int64) *groupChunkSeriesIterator {
	its := make([]chunkenc.Iterator, 0, len(cs))
	for _, chks := range cs {
		its = append(its, chks[0].Chunk.IteratorGroup(mint, int(chks[0].SeriesRef)))
	}
	var it GroupSeriesIterator
	if len(dranges) > 0 {
		it = &groupDeletedIterator{its: its, intervals: dranges}
	} else {
		it = &groupChunkSeriesIteratorNoItvls{its: its}
	}
	return &groupChunkSeriesIterator{
		chunks:    cs,
		i:         0,
		cur:       it,
		mint:      mint,
...
func (it *groupChunkSeriesIterator) nextColumn() {
	its := make([]chunkenc.Iterator, 0, len(it.chunks))
	for _, chks := range it.chunks {
		its = append(its, chks[0].Chunk.IteratorGroup(it.mint, int(chks[0].SeriesRef)))
	}
	if len(it.intervals) > 0 {
		it.cur = &groupDeletedIterator{its: its, intervals: it.intervals}
	} else {
		it.cur = &groupChunkSeriesIteratorNoItvls{its: its}
	}
}

func (it *groupChunkSeriesIterator) At() (int64, []float64) {
	return it.cur.At()
}

func (it *groupChunkSeriesIterator) Next() bool {
	if it.cur.Next() {
		t, _ := it.cur.At()
...
func (it *groupChunkSeriesIterator) Seek(t int64) bool {
	if t > it.maxt {
		return false
	}
	...
	for ; it.chunks[0][it.i].MaxTime < t; it.i++ {
		if it.i == len(it.chunks[0])-1 {
			return false
		}
	}
...
// TODO(Alec).
func (it *groupChunkSeriesIterator) Err() error {
	return nil
}

// Implement GroupSeriesIterator.
type groupChunkSeriesIteratorNoItvls struct {
	i         int
...
func (it *groupChunkSeriesIteratorNoItvls) itsAt() {
	it.values = it.values[:0]
	it.timestamp, _ = it.its[0].At()
...
func (it *groupChunkSeriesIteratorNoItvls) At() (int64, []float64) {
	return it.timestamp, it.values
}

func (it *groupChunkSeriesIteratorNoItvls) Next() bool {
	for _, i := range it.its {
		if !i.Next() {
			return false
		}
...
func (it *groupChunkSeriesIteratorNoItvls) Seek(x int64) bool {
	for _, i := range it.its {
		if !i.Seek(x) {
			return false
		}
...
// TODO(Alec).
func (it *groupChunkSeriesIteratorNoItvls) Err() error {
	return nil
}

// Implement GroupSeriesIterator.
type groupDeletedIterator struct {
	its []chunkenc.Iterator
...
func (it *groupDeletedIterator) itsAt() {
	it.values = it.values[:0]
	it.timestamp, _ = it.its[0].At()
	for _, i := range it.its {
		_, v := i.At()
...
func (it *groupDeletedIterator) itsNext() bool {
	for _, i := range it.its {
		if !i.Next() {
			return false
		}
...
func (it *groupDeletedIterator) At() (int64, []float64) {
	return it.timestamp, it.values
}

func (it *groupDeletedIterator) Next() bool {
Outer:
	for it.itsNext() {
		for it.i != len(it.intervals) {
			if it.intervals[it.i].inBounds(it.timestamp) {
				continue Outer
			}

			if it.timestamp > it.intervals[it.i].Maxt {
				it.i += 1
				continue
			}
...
func (it *groupDeletedIterator) Seek(x int64) bool {
	for _, i := range it.its {
		if !i.Seek(x) {
			return false
		}
...
// TODO(Alec).
func (it *groupDeletedIterator) Err() error {
	return nil
}
