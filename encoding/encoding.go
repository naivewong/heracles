...
// Decode from the MaxBits().
func FromBits(x uint64, num int) int64 {
	if (x >> uint(num - 1)) == 1 {
		return int64(x | ^((uint64(1) << uint(num)) - 1))
	}
	return int64(x)
}

func MaxFromBitsLen(l int) int64 {
	return (int64(1) << uint(l - 1)) - 1
}

func MinFromBitsLen(l int) int64 {
	return int64(-1) * (int64(1) << uint(l - 1))
}
