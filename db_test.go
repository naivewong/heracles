...
func generateGroupData(mint, maxt, step, devMag int64, seriesNum int) ([]int64, [][]float64) {
	ts := []int64{}
	vals := [][]float64{}
	for t := mint; t < maxt; t += step {
		tempVals := make([]float64, seriesNum)
		for i := 0; i < seriesNum; i++ {
			tempVals[i] = rand.Float64()
		}
		ts = append(ts, t + int64(rand.Float64() * float64(devMag)))
...
func verifyRange(t testing.TB, ts []int64, vals [][]float64, idx int, mint, maxt int64, it SeriesIterator) {
	i := 0
	for i < len(ts) {
		if ts[i] >= mint {
			break
	...
	for i < len(ts) && it.Next() {
		timestamp, value := it.At()
		testutil.Equals(t, ts[i], timestamp)
		testutil.Equals(t, vals[i][idx], value)
		i++
	}
	testutil.Equals(t, false, it.Next())
	testutil.Equals(t, true, i >= len(ts) || ts[i] > maxt)
}

func TestGroupDBAll(t *testing.T) {
	{
		// Test 4 hours data.
		ts1, vals1 := generateGroupData(0, 4 * 3600 * 1000, 1000, 100, 3)
		ts2, vals2 := generateGroupData(0, 4 * 3600 * 1000, 1000, 100, 3)
		ts3, vals3 := generateGroupData(0, 4 * 3600 * 1000, 1000, 100, 4)
		
		db1, delete := openTestDB(t, nil)
		defer func() {
			delete()
		}()
		lsets1 := []labels.Labels{
			{{"a", "1"}, {"b", "1"}},
			{{"a", "2"}, {"c", "1"}},
			{{"a", "3"}, {"d", "1"}},
		}
		lsets2 := []labels.Labels{
			{{"a", "4"}, {"e", "1"}},
			{{"a", "5"}, {"f", "1"}},
			{{"a", "6"}, {"g", "1"}},
		}
		lsets3 := []labels.Labels{
			{{"a", "10"}, {"k", "1"}},
			{{"a", "7"}, {"h", "1"}},
			{{"a", "8"}, {"i", "1"}},
			{{"a", "9"}, {"j", "1"}},
		...
		for i := 0; i < len(ts1); i++ {
			app := db1.Appender()
			_, err := app.AddGroup(lsets1, ts1[i], vals1[i])
			testutil.Ok(t, err)
			_, err = app.AddGroup(lsets2, ts2[i], vals2[i])
			testutil.Ok(t, err)
			_, err = app.AddGroup(lsets3, ts3[i], vals3[i])
			testutil.Ok(t, err)
		...
		// Querier.
		q, err := db1.Querier(0, 4 * 3600 * 1000)
		testutil.Ok(t, err)
		...
		for ss.Next() {
			s := ss.At()
			testutil.Equals(t, labels.Labels{{"a", "1"}, {"b", "1"}}, s.Labels())
			it := s.Iterator()
			i := 0
			for it.Next() {
				t_, v := it.At()
				testutil.Equals(t, ts1[i], t_)
				testutil.Equals(t, vals1[i][0], v)
				i += 1
			}
			testutil.Equals(t, len(ts1), i)
			numSeries += 1
		}
		testutil.Equals(t, 1, numSeries)
		q.Close()

		tmpdir := db1.dir
		db1.Close()

		db2, err := Open(tmpdir, nil, nil, nil)
		testutil.Ok(t, err)
		deletePoint := len(ts1) / 4
		
		// Delete().
		err = db2.Delete(ts1[deletePoint], 4*3600*1000, labels.NewEqualMatcher("a", "1"))
		testutil.Ok(t, err)
		err = db2.Delete(0, ts3[deletePoint], labels.NewEqualMatcher("k", "1"))
		testutil.Ok(t, err)
		...
		// Querier.
		q, err = db2.Querier(0, 4 * 3600 * 3600)
		testutil.Ok(t, err)
		ss, err = q.Select(labels.NewEqualMatcher("a", "3"))
		testutil.Ok(t, err)
		numSeries = 0
		for ss.Next() {
			s := ss.At()
			testutil.Equals(t, labels.Labels{{"a", "3"}, {"d", "1"}}, s.Labels())
			it := s.Iterator()
			i := 0
			for it.Next() {
				t_, v := it.At()
				testutil.Equals(t, ts1[i], t_)
				testutil.Equals(t, vals1[i][2], v)
				i += 1
			}
			testutil.Equals(t, deletePoint, i)
			numSeries += 1
		}
		testutil.Equals(t, 1, numSeries)
		q.Close()
		...
		q, err = db2.Querier(0, 4 * 3600 * 3600)
		testutil.Ok(t, err)
		ss, err = q.Select(labels.NewEqualMatcher("h", "1"))
		testutil.Ok(t, err)
		numSeries = 0
		for ss.Next() {
			s := ss.At()
			testutil.Equals(t, labels.Labels{{"a", "7"}, {"h", "1"}}, s.Labels())
			it := s.Iterator()
			i := deletePoint + 1
			for it.Next() {
				t_, v := it.At()
				testutil.Equals(t, ts3[i], t_)
				testutil.Equals(t, vals3[i][1], v)
				i += 1
			}
			testutil.Equals(t, len(ts3), i)
			numSeries += 1
		}
		testutil.Equals(t, 1, numSeries)
		q.Close()
	...
	{
		// Test 20 hours data.
		ts1, vals1 := generateGroupData(0, 20 * 3600 * 1000, 1000, 100, 3)
		...
		{
			// Querier.
			q, err := db1.Querier(0, 20 * 3600 * 1000)
			testutil.Ok(t, err)
			ss, err := q.Select(labels.NewEqualMatcher("a", "1"))
			testutil.Ok(t, err)
			numSeries := 0
		...
		// Reopen the db.
		db1, err := Open(tmpdir, nil, nil, nil)
		testutil.Ok(t, err)
		...
		{
			// Many Queriers (query every 3500 points).
			for i := 0; i < len(ts1); i += 3500 {
				end := i + 3500
				if end > len(ts1) {
					end = len(ts1) - 1
				}
				q, err := db1.Querier(ts1[i], ts1[end])
				testutil.Ok(t, err)
				ss, err := q.Select(labels.NewEqualMatcher("c", "1"))
				testutil.Ok(t, err)
				numSeries := 0
				for ss.Next() {
					s := ss.At()
					testutil.Equals(t, labels.Labels{{"a", "2"}, {"c", "1"}}, s.Labels())
					it := s.Iterator()
					j := i
					for it.Next() {
						t_, v := it.At()
						testutil.Equals(t, ts1[j], t_)
						testutil.Equals(t, vals1[j][1], v)
						j += 1
					}
					testutil.Equals(t, end + 1, j)
					numSeries += 1
				}
				testutil.Equals(t, 1, numSeries)
				q.Close()
