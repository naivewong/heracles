...
type IndexWriter interface {
	...
	// are added later.
	AddSeries(ref uint64, l labels.Labels, chunks ...chunkenc.Meta) error
	...
	// WriteGroupPostings writes all series entries offsets in a group.
	// NOTE(Alec), the order of writing groups should be sorted.
	WriteGroupPostings(it index.Postings) error
...
type IndexReader interface {
	...
	GroupPostings(ref uint64) (index.Postings, error)
	...
	SortedGroupPostings(index.Postings) index.Postings

	// Series populates the given labels and chunk metas for the series identified
	// by the reference.
	// Returns ErrNotFound if the ref does not resolve to a known series.
	Series(ref uint64, lset *labels.Labels, chks *[]chunkenc.Meta) error
...
type ChunkWriter interface {
	...
	WriteChunks(chunks ...chunkenc.Meta) error
...
// 1. Get the corresponding group postings entry.
// 2. Pass ALL_GROUP_POSTINGS, get offsets of all group postings entries.
func (r blockIndexReader) GroupPostings(ref uint64) (index.Postings, error) {
	p, err := r.ir.GroupPostings(ref)
	return p, errors.Wrapf(err, "block: %s", r.b.Meta().ULID)
}
...
// NOTE(Alec), this is not to sort the content inside a group postings entry but sort all the groups by their first series' labels.
func (r blockIndexReader) SortedGroupPostings(p index.Postings) index.Postings {
	return r.ir.SortedGroupPostings(p)
}

func (r blockIndexReader) Series(ref uint64, lset *labels.Labels, chks *[]chunkenc.Meta) error {
	return errors.Wrapf(
...
Outer:
	for p.Next() {
		...
		// NOTE(Alec), cannot directly check if overlapped with [chks[0].MinTime, chks[len(chks)-1].MaxTime] because 
		// [mint, maxt] may lay between the gaps of the chunks.
		for _, chk := range chks {
			if chk.OverlapsClosedInterval(mint, maxt) {
				// Delete only until the current values and not beyond.
				tmin, tmax := clampInterval(mint, maxt, chks[0].MinTime, chks[len(chks)-1].MaxTime)
				tb, _ := stones.Get(chk.LogicalGroupRef)
				if tb == nil {
					stones.addInterval(chk.LogicalGroupRef, Interval{tmin, tmax})
				}
				continue Outer
			}
		}
