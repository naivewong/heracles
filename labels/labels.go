...
func (g *ByGroup) Len() int { return len(*g.Lsets) / g.GSize }
func (g *ByGroup) At(i int) ([]Labels, error) { return (*g.Lsets)[i : i+g.GSize], nil }
func (g *ByGroup) Swap(i, j int) {
	i *= g.GSize
	j *= g.GSize
	for k := 0; k < g.GSize; k++ {
		temp := (*g.Lsets)[i+k]
		(*g.Lsets)[i+k] = (*g.Lsets)[j+k]
		(*g.Lsets)[j+k] = temp
	}
}
func (g *ByGroup) Less(i, j int) bool { return Compare((*g.Lsets)[i*g.GSize], (*g.Lsets)[j*g.GSize]) < 0 }
