...
func createIdxChkReaders(t *testing.T, tc []seriesSamples) (IndexReader, ChunkReader, int64, int64) {
	...
	for i, s := range tc {
		...
		metas := make([]chunkenc.Meta, 0, len(s.chunks))
...
func (m *mockChunkSeriesSet) At() (labels.Labels, []chunkenc.Meta, Intervals) {
...
func (m mockIndex) WriteGroupPostings(it index.Postings) error {
	ep, err := index.ExpandPostings(it)
	if err != nil {
		return err
	}
...
func (m mockIndex) GroupPostings(ref uint64) (index.Postings, error) {
	if ref == index.AllGroupPostings {
		return index.NewIncrementPostings(0, len(m.group)), nil
	}
	if int(ref) >= len(m.group) {
		return nil, errors.Errorf("Fail to read group postings, group index:%d not existed", ref)
	}
	return index.NewListPostings(m.group[ref]), nil
}
...
func (m mockIndex) SortedGroupPostings(p index.Postings) index.Postings {
	ep, err := index.ExpandPostings(p)
	if err != nil {
		return index.ErrPostings(errors.Wrap(err, "expand postings"))
	}
		...
		for _, overlapPercentage := range c.overlapPercentages {
			...
			b.Run(benchMsg, func(b *testing.B) {
				...
				for i := int64(0); i < int64(c.numBlocks); i++ {
					...
					block, err := OpenBlock(nil, createBlock(b, dir, generatedSeries, 10), nil)
					testutil.Ok(b, err)
...
func TestPostingsForMatchers(t *testing.T) {
	...
	for _, c := range cases {
		...
		for p.Next() {
			lbls := labels.Labels{}
			testutil.Ok(t, ir.Series(p.At(), &lbls, &[]chunkenc.Meta{}))
...
func TestClose(t *testing.T) {
	...
	createBlock(t, dir, genSeries(1, 1, 0, 10), 1)
	createBlock(t, dir, genSeries(1, 1, 10, 20), 1)
	...
	for title, selectors := range cases {
		for _, nSeries := range []int{10} {
			for _, nSamples := range []int64{1000, 10000, 100000} {
				...
				qs := []Querier{}
				for x := 0; x <= 10; x++ {
					block, err := OpenBlock(nil, createBlock(b, dir, series, 1), nil)
					testutil.Ok(b, err)
