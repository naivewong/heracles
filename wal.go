...
// RefGroupSeries is a group of RefSeries with the group ID.
type RefGroupSeries struct {
	GroupRef uint64
	Series   []RefSeries
}
...
// RefGroupSample comtains all values of the group at one timestamp.
type RefGroupSample struct {
	GroupRef uint64
	Ids      []uint64
	T        int64
	Vals     []float64
