...
func (m mockIndex) AddSeries(ref uint64, l labels.Labels, chunks ...chunkenc.Meta) error {
...
func (m mockIndex) WriteGroupPostings(it Postings) error {
	ep, err := ExpandPostings(it)
	if err != nil {
		return err
	}
	c := uint64(len(m.group))
	m.group[c] = ep
	for _, p := range ep {
		for i := 0; i < len(m.series[p].chunks); i++ {
			m.series[p].chunks[i].LogicalGroupRef = c
		}
	}
	return nil
}
...
func (m mockIndex) GroupPostings(ref uint64) (Postings, error) {
	...
	return NewListPostings(m.group[ref]), nil
}
...
func (m mockIndex) SortedGroupPostings(p Postings) Postings {
	ep, err := ExpandPostings(p)
	if err != nil {
		return ErrPostings(errors.Wrap(err, "expand postings"))
	}
	...
	sort.Slice(ep, func(i, j int) bool {
		return labels.Compare(m.series[m.group[ep[i]][0]].l, m.series[m.group[ep[j]][0]].l) < 0
	})
	return NewListPostings(ep)
}

func (m mockIndex) Series(ref uint64, lset *labels.Labels, chks *[]chunkenc.Meta) error {
	s, ok := m.series[ref]
	if !ok {
...
func TestIndexRW_Postings(t *testing.T) {
	...
	err = iw.WriteGroupPostings(newListPostings(1, 2))
	err = iw.WriteGroupPostings(newListPostings(3, 4))
	testutil.Ok(t, err)	
...
func TestPersistence_index_e2e(t *testing.T) {
	...
	lbls, err := labels.ReadLabels(filepath.Join("..", "testdata", "20kseries.json"), 10000)
	testutil.Ok(t, err)
	...
	// Put groups.
	for i := 0; i < len(input); i += seriesPerGroup {
		temp := []uint64{}
	...
	for _, g := range groups {
		err = iw.WriteGroupPostings(NewListPostings(g))
		testutil.Ok(t, err)
		mi.WriteGroupPostings(NewListPostings(g))
	}
	...
	for p := range mi.postings {
		...
		var chks, expchks []chunkenc.Meta
	...
	// Verify group postings.
	for i := 0; i < len(groups); i++ {
		gotp, err := ir.GroupPostings(uint64(i))
		testutil.Ok(t, err)

		expp, err := mi.GroupPostings(uint64(i))
		testutil.Ok(t, err)

		p1, _ := ExpandPostings(gotp)
		p2, _ := ExpandPostings(expp)

		var lset, explset labels.Labels
		var chks, expchks []chunkenc.Meta

		testutil.Equals(t, len(p2), len(p1)) // Check if group size equal.
		for i, ref := range p1 {
			err := ir.Series(ref, &lset, &chks)
			testutil.Ok(t, err)

			err = mi.Series(p2[i], &explset, &expchks)
			testutil.Ok(t, err)
			testutil.Equals(t, explset, lset)
			testutil.Equals(t, expchks, chks)
		}
