...
var AllGroupPostings = uint64(math.MaxUint64)
...
type uvarintPostings struct {
	dec *encoding.Decbuf
	cur uint64
}

func newUvarintPostings(dec *encoding.Decbuf) *uvarintPostings {
	return &uvarintPostings{dec: dec}
}

func (it *uvarintPostings) At() uint64 {
	return it.cur
}

func (it *uvarintPostings) Next() bool {
	it.cur = it.dec.Uvarint64()
	if it.dec.Err() == nil {
		return true
	}
	return false
...
// This may not be used because of inefficiency.
func (it *uvarintPostings) Seek(x uint64) bool {
	if it.cur >= x {
		return true
	}
	for it.Next() {
...
func (it *uvarintPostings) Err() error {
	return nil
}

type incrementPostings struct {
	start  int
	end    int
	i      int
}

func NewIncrementPostings(start int, end int) *incrementPostings {
	return &incrementPostings{start: start, end: end, i: start - 1}
}

func (it *incrementPostings) At() uint64 {
	return uint64(it.i)
}

func (it *incrementPostings) Next() bool {
	it.i += 1
	if it.i != it.end {
		return true
	}
...
func (it *incrementPostings) Seek(x uint64) bool {
	if int(x) < it.start || int(x) >= it.end {
		return false
	}
	it.i = int(x)
	return true
...
func (it *incrementPostings) Err() error {
	return nil
}
